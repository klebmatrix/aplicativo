...
# --- FUNÇÃO QUE CRIA A TABELA AUTOMATICAMENTE ---
@app.before_request
def create_tables():
    conn = get_db_connection()
    if conn:
        cur = conn.cursor()
        # Este comando cria a tabela exatamente como o sistema precisa
        cur.execute('''
...
<head>
    ...
    <style>
        ...
        .box { max-width: 500px; margin: auto; border: 1px solid #ddd; padding: 30px; border-radius: 10px; }
        input { width: 100%; padding: 12px; margin: 10px 0; box-sizing: border-box; }
        .btn { width: 100%; padding: 12px; background: #000; color: #fff; border: none; cursor: pointer; font-weight: bold; }
        .card { border: 1px solid #eee; padding: 15px; margin-top: 10px; display: flex; justify-content: space-between; align-items: center; }
        @media print { .no-print { display: none; } .print-area { display: block !important; } }
        .print-area { display: none; text-align: center; border: 10px double #000; padding: 50px; }
    </style>
</head>
<body>
    <div class="box no-print">
        {% if admin %}
            ...
            <button class="btn" onclick="cadastrar()">CADASTRAR CLIENTE</button>
        {% else %}
            <div id="login">
                <h2>LOGIN CLIENTE</h2>
            ...
            <div id="dash" style="display:none;">
                ...
                <button class="btn" style="background: #2563eb;" onclick="gerar()">GERAR CHAVE</button>
                <div id="lista"></div>
            </div>
        {% endif %}
    </div>
    ...
    async function cadastrar() {
        ...
        if(res.ok) alert("Sucesso! Tabela criada e cliente salvo.");
        else alert("Erro: Verifique se o DATABASE_URL está correto no Render.");
    }
    ...
    async function entrar() {
        ...
        d.hist.reverse().forEach(t => {
            const p = t.split(' | ');
            h += `<div class="card"><span>${p[0]}<br><b>${p[1]}</b></span><button onclick="imprimir('${t}')">VER</button></div>`;
        });
