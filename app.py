...
if st.session_state.preview_questoes and st.session_state.sub_menu in ["op", "eq", "col", "alg", "man"]:
    ...
    # --- LÓGICA DE DOWNLOAD (DOIS BOTÕES) ---
    ...
    b1, b2 = st.columns(2)
    with b1:
        st.download_button("📄 Sem Cabeçalho", criar_pdf(False), "atividade_limpa.pdf", "application/pdf", use_container_width=True)
    with b2:
        st.download_button("🖼️ Com Cabeçalho", criar_pdf(True), "atividade_cabecalho.pdf", "application/pdf", use_container_width=True)