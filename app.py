...
<head>
    ...
    <style>
        ...
        .container { max-width: 1100px; margin: auto; background: var(--card); padding: 25px; border-radius: 15px; border: 1px solid #334155; }
        
        /* ADMIN */
        table { width: 100%; border-collapse: collapse; margin-top: 20px; background: #0f172a; }
        th { background: #1e293b; color: var(--blue); padding: 12px; text-align: left; border-bottom: 2px solid var(--blue); }
        td { padding: 12px; border-bottom: 1px solid #334155; }
        .btn { padding: 8px 12px; border: none; border-radius: 6px; cursor: pointer; font-weight: bold; color: white; margin: 2px; transition: 0.2s; }
        ...
        .hist-item { background: #16213e; padding: 15px; margin-top: 10px; border-radius: 10px; display: flex; justify-content: space-between; align-items: center; cursor: pointer; }
        .hist-item.selected { border: 1px solid var(--gold); background: #1e3a8a; }

        @media print {
            .no-print { display: none !important; }
            body { background: white !important; color: black !important; }
            .certificado { display: block !important; border: 8px double #000; padding: 40px; text-align: center; margin-bottom: 50px; page-break-inside: avoid; }
...
<body>
    <div class="container">
        {% if tipo == 'admin' %}
            <h1 style="color:var(--blue)">PAINEL DE CONTROLE MESTRE</h1>
            <div class="no-print">
                <input type="password" id="mestre" placeholder="Chave Admin">
                <button class="btn" style="background:var(--blue)" onclick="listar()">CONECTAR BANCO</button>
            </div>
            ...
            <div id="form_add" class="no-print" style="margin-top:20px; padding:15px; background:#0f172a; border-radius:10px;">
                ...
                <input type="text" id="p" placeholder="Senha/PIN (até 8 car.)" maxlength="8">
                <input type="number" id="l" placeholder="Qtd Créditos (ex: 10)">
                <button class="btn" style="background:var(--green)" onclick="cadastrar()">CADASTRAR</button>
            </div>
        ...
        {% else %}
            <div id="login_area" style="text-align:center; padding: 50px 0;">
                <h1 style="letter-spacing:3px">QUANTUM AUTH</h1>
                <input type="password" id="pin_cli" placeholder="PIN DE ATÉ 8 DÍGITOS" maxlength="8" style="width:280px; text-align:center; font-size:20px;">
                <br><br>
            ...
            <div id="dashboard" style="display:none;">
                ...
                <div class="box-gerar no-print">
                    <input type="text" id="obs" placeholder="Referência do Código" style="width:60%">
    ...
    async function listar() {
        ...
        let h = `<table><tr><th>Empresa</th><th>PIN / Senha</th><th>Plano</th><th>Ações</th></tr>`;
        dados.forEach(c => {
            const isVip = c.l === -1;
            h += `<tr>
                <td><b>${c.n}</b></td>
                <td style="font-family:monospace; color:var(--gold)">${c.p}</td>
                <td>${isVip?'ASSINATURA':c.u+' / '+c.l}</td>
                <td>
                    <button class="btn" style="background:var(--blue)" onclick="mudarPin('${c.p}')">Mudar PIN</button>
                    <button class="btn" style="background:var(--gold); color:black" onclick="setVip('${c.p}')">VIP</button>
                    <button class="btn" style="background:#475569" onclick="addCr('${c.p}')">Saldo</button>
    ...
    async function mudarPin(p) {
        const np = prompt("Digite o novo PIN (até 8 caracteres):");
        if(np) {
            await fetch('/admin/mudar-pin', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({key:document.getElementById('mestre').value, pin_antigo:p, pin_novo:np})});
            listar();
        }
    }
    ...
    async function addCr(p) { const q=prompt("Novo limite total:"); if(q) await fetch('/admin/credito', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({key:document.getElementById('mestre').value, pin:p, qtd:q})}); listar(); }
    async function del(p) { if(confirm("Remover cliente?")) await fetch('/admin/deletar', {method:'DELETE', headers:{'Content-Type':'application/json'}, body:JSON.stringify({key:document.getElementById('mestre').value, pin:p})}); listar(); }
    ...
    async function gerar() {
        ...
        if(res.ok) entrar(); else alert("Limite atingido ou erro!");
    }
    </script>
</body>
</html>
...
@app.route('/admin/mudar-pin', methods=['POST'])
def change_pin():
    d = request.json
    if d.get('key', '').strip() != get_admin_key(): return jsonify({"e":403}), 403
    conn = get_db_connection(); cur = conn.cursor()
    cur.execute("UPDATE clientes SET pin_hash = %s WHERE pin_hash = %s", (d['pin_novo'], d['pin_antigo']))
