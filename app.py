...
def tratar_math(texto):
    ...
    t = texto.strip()
    # Converte x² para x^{2} e trata raízes se necessário
...
def validar_acesso(pin_digitado):
    try:
        ...
        # Chave mestra sempre em minúsculo conforme sua regra
...
if perfil == "admin":
    ...
    # --- MÓDULOS DE GERADORES (ALIMENTAM O PREVIEW) ---
    if op_atual == "op":
        st.header("🔢 Operações")
        escolhas = st.multiselect("Sinais:", ["+", "-", "x", "÷"], ["+", "-"])
        qtd = st.number_input("Qtd:", 4, 30, 10)
        if st.button("Gerar Preview"):
            # Adiciona título automático conforme suas regras
...
if st.session_state.preview_questoes:
    st.divider()
    # Cabeçalho sempre no topo conforme solicitado
    ...
    for q in st.session_state.preview_questoes:
        ...
        # Seção Numerada (Reseta letras para 'a')
    ...
    if st.button("📥 Baixar Atividade Finalizada (PDF)"):
        ...
        for q in st.session_state.preview_questoes:
            ...
            if line.lower().startswith(("t.", "titulo:", "título:")):
                ...
                pdf.ln(5); pdf.set_font("Arial", 'B', 12); pdf.multi_cell(0, 8, clean_txt(line)); l_pdf_idx = 0
            else:
                pdf.set_font("Arial", size=11)
                pdf.multi_cell(0, 8, f"{letras[l_pdf_idx%26]}) {clean_txt(line)}")
                l_pdf_idx += 1
