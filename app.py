...
<body>
    <div class="container no-print">
        {% if admin %}
            ...
            <input type="number" id="l" placeholder="Qtd. de Créditos (Ex: 100)" value="100">
            <button class="btn btn-black" onclick="cadastrar()">CADASTRAR E DEFINIR CRÉDITOS</button>
        ...
        {% else %}
            <div id="login">
                ...
                <input type="password" id="pass" placeholder="Digite sua Senha" minlength="6" maxlength="8">
                <button class="btn btn-black" onclick="entrar()">ACESSAR SISTEMA</button>
            </div>
            <div id="dash" style="display:none;">
                <h3 id="cli_nome" style="color:#2563eb;"></h3>
                <p><b>Créditos Disponíveis:</b> <span id="cli_limite"></span></p>
                <input type="text" id="prod" placeholder="Referência/Lote do Equipamento">
                <button class="btn btn-black" onclick="gerar()">GERAR E AUTENTICAR</button>
    ...
    async function cadastrar() {
        ...
        if(p.length < 6 || p.length > 8) return alert("A senha deve ter entre 6 e 8 caracteres!");
        
        await fetch('/admin/cadastrar', {
            method:'POST', headers:{'Content-Type':'application/json'},
            body: JSON.stringify({
                empresa: document.getElementById('n').value, 
                pin: p, 
                limite: document.getElementById('l').value
            })
        });
    ...
    async function listar() {
        ...
        let h = "<table><tr><th>Empresa</th><th>Senha</th><th>Créditos</th><th>Excluir</th></tr>";
        dados.forEach(c => {
            h += `<tr><td>${c.empresa}</td><td>${c.pin_hash}</td><td>${c.limite}</td>
            <td><button class="btn btn-red" onclick="excluir('${c.pin_hash}')">REMOVER</button></td></tr>`;
        });
    ...
    async function entrar() {
        ...
        if(!res.ok) return alert("Senha não encontrada!");
    ...
    async function gerar() {
        ...
        if(res.status === 403) alert("Créditos esgotados!");
        else entrar();
    }

    function imprimir(texto) {
        const p = texto.split(' | ');
        document.getElementById('cert').innerHTML = `<h1>CERTIFICADO</h1><br><h2>${p[1]}</h2><br><p>CHAVE DE AUTENTICAÇÃO:</p><h3>${p[2]}</h3><p>DATA: ${p[0]}</p>`;
        window.print();
    }
